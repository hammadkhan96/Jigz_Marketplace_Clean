...
import { getPasswordResetUrl } from "./config/environment";
...
// ============================================================================
// 
// NEW ERROR HANDLING UTILITIES AVAILABLE:
// 
// 1. Use asyncHandler to wrap async route handlers:
//    app.get('/path', asyncHandler(async (req, res) => { ... }))
// 
// 2. Use custom error classes for specific error types:
//    import { ValidationError, NotFoundError, DatabaseError } from '../utils/errorUtils';
...
//    import { dbLogger, authLogger, apiLogger } from '../utils/logger';
//    dbLogger.error('Database operation failed', { table: 'users', operation: 'insert' });
// 
// 5. Use database error handling wrapper:
...
/**
 * Moderator middleware - Ensures user has moderator or admin role
 * (can access job management but not user management or coins)
 */
...
// ============================================================================
// These functions combine common middleware patterns to reduce code duplication
// and make route definitions more readable and maintainable.
//
// Benefits:
// - DRY principle: Don't Repeat Yourself
// - Consistent middleware application across routes
// - Easier to modify middleware behavior globally
// - Better code readability and maintainability
// - Reduced chance of middleware ordering errors

/**
 * Combines authentication with file upload for multiple images
 * @param fieldName - The form field name for images (default: "images")
 * @param maxCount - Maximum number of images (default: 5)
 */
function withAuthAndImages(fieldName: string = "images", maxCount: number = 5) {
  return [requireAuth, upload.array(fieldName, maxCount)];
}

/**
 * Combines authentication with single file upload
 * @param fieldName - The form field name for the file (default: "profileImage")
 */
function withAuthAndSingleFile(fieldName: string = "profileImage") {
  return [requireAuth, upload.single(fieldName)];
}

/**
 * Combines authentication with admin role requirement
 */
function withAuthAndAdmin() {
  return [requireAuth, requireAdmin];
}

/**
 * Combines authentication with moderator role requirement
 */
function withAuthAndModerator() {
  return [requireAuth, requireModerator];
}

/**
 * Combines authentication with admin role requirement for admin-only operations
 */
...
/**
 * Combines authentication with file upload for profile images
 */
function withAuthAndProfileImage() {
  return [requireAuth, upload.single("profileImage")];
}

/**
 * Combines authentication with file upload for job images
 */
function withAuthAndJobImages() {
  return [requireAuth, upload.array("images", 5)];
}

/**
 * Combines authentication with file upload for service images
 */
function withAuthAndServiceImages() {
  return [requireAuth, upload.array("images", 5)];
}

/**
 * Combines authentication with file upload for object uploads
 */
function withAuthAndObjectUpload() {
  return [requireAuth];
}

/**
 * Combines authentication with admin role for user management operations
 */
...
/**
 * Combines authentication with moderator role for content moderation
 */
function withContentModeration() {
  return [requireAuth, requireModerator];
}

/**
 * Combines authentication with admin role for system administration
 */
function withSystemAdmin() {
  return [requireAuth, requireAdmin];
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ============================================================================
  // Routes for handling email verification tokens and resending verification emails
  ...
  // ============================================================================
  // FILE UPLOAD & STATIC ENDPOINTS
  // ============================================================================
  // Routes for handling file uploads and serving static assets
  ...
  // ============================================================================
  // Routes for creating, updating, searching, and managing job postings
  
  // Job routes
  // Advanced search endpoint with pagination and full-text search
  app.get("/api/search/jobs", cacheMiddleware.searchResults, async (req, res) => {
    try {
  ...
  app.get("/api/jobs", cacheMiddleware.jobListings, async (req, res) => {
    try {
  ...
  app.get("/api/jobs/:id", cacheMiddleware.jobListings, async (req, res) => {
    try {
  ...
  app.post("/api/jobs", ...withAuthAndJobImages(), async (req, res) => {
    try {
      const userId = (req.user as any).id;
  ...
  // Update job
  app.put("/api/jobs/:id", ...withAuthAndJobImages(), async (req, res) => {
    try {
      const userId = (req.user as any).id;
  ...
  // ============================================================================
  // Routes for managing job applications (apply, view, accept, reject)
        ...
        if (job && applicant) {
          ...
          if (applicant.email && jobPoster) {
            ...
            try {
              ...
              // Missing applicant email or job poster for application acceptance notification
            }
        }
      }
  ...
  // ============================================================================
  // Routes for creating and managing reviews/ratings for jobs and services
  ...
  // ============================================================================
  // Routes for managing conversations and messages between users
  ...
  // ============================================================================
  // Routes for administrative functions and content moderation
  
  // Admin routes
  app.get("/api/admin/users", ...withAdminOnly(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/ban", ...withAdminOnly(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/unban", ...withAdminOnly(), async (req, res) => {
    try {
  ...
  app.delete("/api/admin/users/:id", ...withAdminOnly(), async (req, res) => {
    try {
  ...
  app.get("/api/admin/jobs/pending", ...withAuthAndModerator(), async (req, res) => {
    try {
  ...
  app.get("/api/admin/services/pending", ...withAuthAndModerator(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/jobs/:id/approve", ...withAuthAndModerator(), async (req, res) => {
    try {
      ...
      if (jobPoster && jobPoster.email) {
        ...
        try {
          ...
          // No job poster or email found for notification
        }
  ...
  app.patch("/api/admin/jobs/:id/reject", ...withAuthAndModerator(), async (req, res) => {
    try {
      const userId = (req.user as any)?.id || "admin";
      const jobId = req.params.id;
  ...
  app.patch("/api/admin/services/:id/approve", ...withAuthAndModerator(), async (req, res) => {
    try {
      const userId = (req.user as any)?.id || "admin";
  ...
  app.patch("/api/admin/services/:id/reject", ...withAuthAndModerator(), async (req, res) => {
    try {
      const userId = (req.user as any)?.id || "admin";
      const serviceId = req.params.id;
  ...
  app.patch("/api/admin/users/:id/promote", ...withUserManagement(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/demote", ...withUserManagement(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/promote-moderator", ...withUserManagement(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/demote-moderator", ...withUserManagement(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/change-password", ...withUserManagement(), async (req, res) => {
    try {
  ...
  app.get("/api/admin/stats", ...withSystemAdmin(), async (req, res) => {
    try {
  ...
  app.delete("/api/admin/jobs/:id", ...withSystemAdmin(), async (req, res) => {
    try {
      const jobId = req.params.id;
  ...
  app.get("/api/admin/users/coins", ...withSystemAdmin(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/coins/add", ...withSystemAdmin(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/users/:id/coins/remove", ...withSystemAdmin(), async (req, res) => {
    try {
      const { amount } = req.body;
      if (!amount || amount <= 0) {
        return res.status(400).json({ message: "Amount must be a positive number" });
  ...
  app.patch("/api/admin/users/:id/coins/set", ...withSystemAdmin(), async (req, res) => {
    try {
      const { amount } = req.body;
      if (amount < 0) {
  ...
  app.post("/api/admin/apply-coin-caps", ...withSystemAdmin(), async (req, res) => {
    try {
  ...
  app.post("/api/admin/users/:userId/subscription", ...withSystemAdmin(), async (req, res) => {
    try {
  ...
  app.patch("/api/user/profile", ...withAuthAndProfileImage(), async (req, res) => {
    try {
      const userId = (req.user as any).id;
  ...
  app.get("/api/admin/reports", ...withContentModeration(), async (req, res) => {
    try {
  ...
  app.get("/api/admin/reports/pending", ...withContentModeration(), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/reports/:reportId", ...withContentModeration(), async (req, res) => {
    try {
  ...
  app.get("/api/users/:userId/skill-endorsements", cacheMiddleware.userProfile, async (req, res) => {
    try {
  ...
  app.use("/uploads", cacheMiddleware.staticAssets, express.static(path.join(process.cwd(), "uploads")));

  // Manual trigger for job expiry (admin only) - for debugging/testing
  app.post("/api/admin/close-expired-jobs", ...withAdminOnly(), async (req, res) => {
    try {
      await storage.closeExpiredJobs();
  ...
  app.get("/public-objects/:filePath(*)", cacheMiddleware.staticAssets, async (req, res) => {
  ...
  app.put("/api/services/:serviceId", ...withAuthAndServiceImages(), async (req, res) => {
    try {
      const { serviceId } = req.params;
      const updates = req.body;
      const userId = (req.user as any).id;
  ...
  // ============================================================================
  // SERVICES & SERVICE REQUESTS ENDPOINTS
  // ============================================================================
  // Routes for managing services offered by users and service requests

  // Get all approved services (with filtering)
  app.get("/api/services", cacheMiddleware.serviceListings, async (req, res) => {
    try {
  ...
  app.get("/api/services/:id", cacheMiddleware.serviceListings, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Create a new service (requires approval) - costs 10 coins
  app.post("/api/services", ...withAuthAndServiceImages(), async (req, res) => {
    try {
      const userId = (req.user as any)?.id;
      ...
      if (user.coins < 10) {
        return res.status(400).json({ 
          message: "Insufficient coins. You need 10 coins to post a service. Coins reset monthly.",
          coinsNeeded: 10,
          coinsAvailable: user.coins
        });
      }
      ...
      await storage.deductCoins(userId, 10);
